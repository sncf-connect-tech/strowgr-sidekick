...
var (
	configFile       = flag.String("config", "conf", "Configuration file")
	...
	deleteChan       = make(chan ReloadEvent, 100)
	lastSyslogReload = time.Now()
	haFactory        *LoadbalancerFactory
...
func main() {
	...
	lumberjack := &lumberjack.Logger{
		Filename:   *logDir + string(filepath.Separator) + "log",
	...
	// initializations
	haFactory = NewLoadbalancerFactory()
	haFactory.Fake = *fake
	haFactory.Properties = properties

	syslog = NewSyslog(properties)
	...
	sdkLogger := SdkLogger{Logrus: nsqlogger}
	...
	// Start consumers
	consumers = NewConsumers(*properties, *config, sdkLogger)
	...
	// Start http API
	restAPI := NewRestApi(properties, consumers)
	go func() {
		defer wg.Done()
		wg.Add(1)
		err := restAPI.Start()
...
func pingNSQD(config Config) (bool, error) {
...
func filteredHandler(event string, message *nsq.Message, f HandlerFunc) error {
	...
	case "delete_requested":
		deleteChan <- ReloadEvent{F: f, Message: data}
	case "commit_requested", "commit_slave_completed":
		...
		select {
		case reloadChan <- ReloadEvent{F: f, Message: data}:
...
func logAndForget(data *EventMessageWithConf) error {
	data.Context().Fields(log.Fields{}).Debug("receive commit completed event")
	return nil
}

func reloadSlave(data *EventMessageWithConf) error {
...
func reloadMaster(data *EventMessageWithConf) error {
	isMaster, err := properties.IsMaster(data.Conf.Bind)
	if err != nil {
		log.WithField("bind", data.Conf.Bind).WithError(err).Info("can't find if binding to vip")
	}
...
func deleteHaproxy(data *EventMessageWithConf) error {
	context := data.Context()
	hap := NewHaproxy(properties, &context)
	err := hap.Stop()
	if err != nil {
		return err
	}
...
func reloadHaProxy(data *EventMessageWithConf, masterRole bool) error {
	context := data.Context()
	var hap Loadbalancer
...
func bodyToData(jsonStream []byte) (*EventMessageWithConf, error) {
	dec := json.NewDecoder(bytes.NewReader(jsonStream))
	var message EventMessageWithConf
	err := dec.Decode(&message)
	return &message, err
}

func publishMessage(topicPrefix string, data interface{}, context Context) error {
